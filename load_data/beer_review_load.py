...
def load_data(data_path):
	...
	data_pd = df[['user', 'user_id', 'item', 'item_id', 'rating', 'review_text']]
	return data_pd
...
def load_data_deepconn(data_path, train_ratio=.8, test_ratio=.2, rebuild=False, random_state=2020, coef=0.0,
	...
	user_ids = pd.unique(data_pd.user_id).tolist()
	item_ids = pd.unique(data_pd.item_id).tolist()
	...
	review_tokens = review.apply(lambda x: split_sentence_to_word_list(str(x)))
	data_pd['review_token_ids'] = review_tokens.apply(lambda x: words2ids(str(x)))
	...
	user_review = data_pd.groupby('user_id')['review_token_ids'] \
		.apply(lambda x: sum(x, [])).apply(lambda x: x[:512] if len(x)>512 else x)
	...
	if mode == 0:
		...
		lmdb_path = os.path.join('/tmp/zli6', dataset, 'NAECF_'+str(random_state), 'mode_4', 'lmdb_user_'+str(coef_u)+'_item_'+str(coef_i))
	elif mode == 5:
