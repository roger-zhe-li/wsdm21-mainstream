...
fig, axes = plt.subplots(1, 3, sharex=True, sharey=True, figsize=(50, 15))
...
# 	print(sstr % ('distribution:', n, sv, ss ,sk))
	...
	axes[i].plot(axis, np.sort(gain_abs), c=colors[i], lw=3, marker=markers[i], markersize=12)
	axes[i].plot(diag, zeros, c='black', lw=1, ls='--')
...
# ax=plt.gca();#获得坐标轴的句柄
	...
	axes[i].spines['top'].set_linewidth(2);####设置上部坐标轴的粗细
...
plt.xlabel('Splits(sorted by performance gain)')
plt.ylabel('Performance Gain')
plt.xticks([])
plt.savefig(os.path.join('figures', 'q2_camera_ready_new' + '.png'),dpi=300)
