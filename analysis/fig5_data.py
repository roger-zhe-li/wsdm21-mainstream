...
for j in range(8):	
	for k in range(1, 11):
		...
		# print(th_10, th_50, th_90)

		for m in range(len(x)):
			if x[m] >= 0 and c_user[m] >= 3:
				if x[m] < th_10 and x[m] >= 0:
					Bin = 0
				if x[m] < th_50 and x[m] >= th_10:
					Bin = 1
				if x[m] < th_90 and x[m] >= th_50:
					Bin = 2
				if x[m] >= th_90:
					Bin = 3
			else:
				Bin = -1
			data.append([seed, weights_u[j], m, c_user[m], x[m], y[m], Bin])

df = pd.DataFrame(data, columns=['seed', 'weight', 'user', 'count', 'uRMSE', 'cos', 'bin'])
print(df.head())
print(np.array(data).shape)		
df.to_csv(os.path.join('res_data', 'Fig5_'+ds+'.csv'), header=True, index=0)	
