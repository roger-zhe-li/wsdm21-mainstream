...
for k in range(1, 11):
	...
	for m in range(len(x)):
		if x[m] >= 0 and c_user[m] >= 3:
			data_item_0.append([z[m], z[m]-x[m]])
			data_t.append([z[m], z[m]-x[m]])
			# data_item_1.append([z[m], z[m]-y[m]])
# print(np.array(data_item_0).shape)
	th_90 = np.quantile(np.array(data_t)[:, 0], 0.9)
	for m in range(len(x)):
		if x[m] >= 0 and z[m] <= th_90:
			data_item_1.append([z[m], z[m]-x[m]])

	for m in range(len(y)):
		if y[m] >= 0 and c_user[m] >= 3:
			data_item_2.append([z[m], z[m]-y[m]])
			data_t_.append([z[m], z[m]-y[m]])

	th_90 = np.quantile(np.array(data_t_)[:, 0], 0.9)
	for m in range(len(y)):
		if y[m] >= 0 and z[m] <= th_90:
			data_item_3.append([z[m], z[m]-y[m]])
...
plt.tight_layout()
plt.savefig(os.path.join('figures', 'q1_'+dataset+'.png'),dpi=300)
