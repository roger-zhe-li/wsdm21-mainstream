...
for i in range(len(datasets)):
	...
	for k in range(1, 11):
		...
		for j in range(8):
			...
			bin0.append(np.mean(np.array(data_bin_0)[:, 1]))
			bin1.append(np.mean(np.array(data_bin_1)[:, 1]))
			bin2.append(np.mean(np.array(data_bin_2)[:, 1]))
			bin3.append(np.mean(np.array(data_bin_3)[:, 1]))

			gains.append(gain)
			gains_90.append(gain_90)
			# bin_0.append(np.mean(np.arraay(data_bin_0)))
			if j == 7:
				...
				for m in range(8):
					bin_0.append(bin0[m]-bin0[0])
					bin_1.append(bin1[m]-bin1[0])
					bin_2.append(bin2[m]-bin2[0])
					bin_3.append(bin3[m]-bin3[0])

				for n in range(8):
					bin_gains.append([dataset, weights_u[n], seeds[k-1], bin_0[n], bin_1[n], bin_2[n], bin_3[n]])
	...
	# plt.plot(axis, np.sort(gain_abs), c='r', lw=2, label='All Data', marker='o')
	...
	# plt.xlabel('Splits (sorted by performance gain)', fontsize=12)
	...
	# plt.xticks([])
	# plt.savefig(os.path.join('figures', 'q2_'+dataset+'.png'),dpi=300)
	...
	# print(bin_2)
	# print(bin_3)
	# print(np.mean(bin_0), np.mean(bin_1), np.mean(bin_2), np.mean(bin_3))


header = ['dataset', 'weight', 'seed', 'bin0', 'bin1', 'bin2', 'bin3']
df = pd.DataFrame(bin_gains)
print(df.shape)
df.to_csv(os.path.join('res_data', save_path), header=header, index=False)


	# header = ['bin0', 'bin1', 'bin2', 'bin3']
